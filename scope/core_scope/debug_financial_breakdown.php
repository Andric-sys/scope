...
$sql = "
  ...
  WHERE j.invoice_date IS NOT NULL
    AND DATE(j.invoice_date) BETWEEN ? AND ?
...
echo "=== FINANCIAL STATUS BREAKDOWN FOR JANUARY 2025 ===\n";
echo str_repeat("-", 100) . "\n";
...
foreach ($rows as $row) {
  printf("%-20s | %15d | %20s | %20s\n", 
    $row['status'] ?? 'NULL',
    $row['total_orders'],
    number_format($row['income_amount'], 2),
...
printf("%-20s | %15d | %20s | %20s\n", 
  "GRAND TOTAL",
  $total['total_orders'],
  number_format($total['income_amount'], 2),

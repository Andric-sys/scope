<?php
// /areas_por_rol_api.php (SOPORTE)
// API JSON para matriz de permisos de áreas (areas_por_rol)
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
  ...
  exit("API endpoint. Abre areas_por_rol.php (vista).");
}
...
try {
  ...
  if ($action === 'meta') {
    ...
    // Mostrar TODAS las áreas para que admin pueda asignarlas
    $rs = $conn->query("SELECT id_area, nombre, descripcion, estatus FROM areas ORDER BY nombre ASC");
    while($r = $rs->fetch_assoc()){
      $areas[] = [
        'id_area'=>(int)$r['id_area'],
        'nombre'=>$r['nombre'],
    ...
    json_out(true, ['roles'=>$roles, 'areas'=>$areas]);
  }
  ...
  if ($action === 'get') {
    ...
    $allowed = [];
    $st = $conn->prepare("SELECT id_area FROM areas_por_rol WHERE id_rol=? AND permitido=1");
    $st->bind_param("i", $id_rol);
    $st->execute();
    $rs = $st->get_result();
    while($r = $rs->fetch_assoc()){
      $allowed[] = (int)$r['id_area'];
    }
  ...
  if ($action === 'save') {
    ...
    // Estrategia: upsert por cada área
    $stUp = $conn->prepare("
      INSERT INTO areas_por_rol (id_rol, id_area, permitido)
      VALUES (?,?,?)
      ON DUPLICATE KEY UPDATE permitido=VALUES(permitido)
    ");

    foreach($permisos as $p){
      $id_area = (int)($p['id_area'] ?? 0);
      $permitido = (int)($p['permitido'] ?? 0);
      if ($id_area <= 0) continue;
      $permitido = ($permitido === 1) ? 1 : 0;

      $stUp->bind_param("iii", $id_rol, $id_area, $permitido);
      $stUp->execute();
    }

    $conn->commit();
    json_out(true, ['msg'=>'Permisos de áreas actualizados']);
  }
